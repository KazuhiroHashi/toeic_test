...
### 4. ビルドと審査提出 — **済(2026年8月11日、審査待ち)**

**2026年8月11日に審査へ提出済み(アプリ本体 1.0.0(13) + 課金アイテム「全セット解放」)。**
- 審査は最大48時間。結果はメールで届く。リジェクトされたら文面をClaudeに貼る
- **承認されてもリリースボタンは押さない**(手動リリース設定済み)。
  組織移行完了でDeveloper名が Bridge AI になってから公開する
- ビルド13の内容:実機フィードバック修正(ズーム無効・解説の聞き直し・予想スコア廃止・
  音量注記の出し分け・iPad二分割・ホーム3列)+新アイコン(TOEIC®総合問題集ロゴ)
- ストア文面ルール:音声は「録音」「ナレーター」と書かない(機械合成のため)。
  審査メモ・説明文の確定版は tools/APPSTORE_HOWTO.md 手順10-2と13
